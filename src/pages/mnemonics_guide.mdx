...
# WalletD Mnemonics {{ className: 'lead' }}
---

# BIP39 Mnemonic

You're probably familiar with how most cryptocurrencies provide you with a 12-25 word secret phrase in order to access your cryptocurrency. That is precisely what a BIP39 mnemonic is. 
...
```rust {{ title: 'Example generating a random BIP39 mnemonic with default settings' }}
// Use this to import the functionality you'll need from WalletD for this section of the guide
use walletd::walletd_bip39::prelude::*;
use walletd::walletd_bip39;

// we are putting our examples inside a main function that returns a `Result` 
// we are going to make use of `?` in the code snippets to handle the error instead of `.unwrap()` 
...
```rust {{ title: 'Show the mnemonic seed as bytes' }}
println!("mnemonic seed as bytes: {:?}", mnemonic.to_seed().as_bytes());
```

```rust {{ title: 'Show the mnemonic seed as a hex string' }}
